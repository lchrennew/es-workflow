...
export default class TargetRequest extends DomainModel {
    ...
    constructor(name, { trigger }, { props, errorTracking, postResponseScript }) {
        super(TargetRequest.kind, name, { trigger }, { props, errorTracking, postResponseScript });
    }
    ...
    async #bind(context) {
        ...
        try {
            const script = `async ({ listener, trigger, method, query, headers, body, props, variables, targetRequest })=>{\
        ${ context.perRequestBindingScript };\
        return variables;}`
            const { bind } = await importNamespace(exportName('bind', script))
            // 展开context和variables是为了防止多个targetRequest互相影响
            ...
            logger.error(error)
            const targetRequestInternalError = new TargetRequestInternalError(error.stack, ...context.chain)
    ...
    async trigger(context, targetInterceptor, targetSystem, template,) {
        ...
        context = {
            ...
            errorTracking: this.spec.errorTracking,
        ...
        try {
            ...
            const targetSystemResponded = await this.#sendTo(targetSystem, context, template);
            if (this.spec.postResponseScript) await this.#postResponseHook({ ...context, ...targetSystemResponded })
            return true;
        } catch (error) {
            logger.error(error)
            const targetRequestInternalError = new TargetRequestInternalError(error.stack, ...context.chain)
    ...
    async #sendTo(targetSystem, context, template) {
        ...
        return targetSystem.commit(request, { ...context, variables })
    }

    async #postResponseHook(context) {
        const script = `async ({content, api, dayjs})=>{${ this.spec.postResponseScript }}`
        const { hook } = await importNamespace(exportName('hook', script))
        await hook(context)
    }
}
