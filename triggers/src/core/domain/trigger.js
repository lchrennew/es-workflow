...
import { TriggerNamespaceReady } from "./events/trigger-namespace-ready.js";
import TargetRequestsCollector from "./target-requests-collector.js";
...
export default class Trigger extends DomainModel {
    ...
    async #getNamespace(context) {
        if (this.spec.namespace) {
            const script = `async ({method, headers, body, query, listener, trigger, variables, api})=>{\
            let namespace; ${ this.spec.namespace }; return namespace;\
            }`
            const { getNamespace } = await importNamespace(exportName('getNamespace', script))
            return await getNamespace(context)
        } else return context.variables['~'] || ''
    }

    async #getTargetRequestsCollector() {
        return ofType(
            await client.getOne(TargetRequestsCollector.kind, this.spec.targetRequestsCollector),
            TargetRequestsCollector)
    }
    ...
    async invoke(context) {
        ...
        try {
            ...
            context.namespace = await this.#getNamespace({ ...context, variables });
            const triggerNamespaceReady
                = new TriggerNamespaceReady(context.namespace, ...context.chain)
            triggerNamespaceReady.flush()
            context.chain = [ ...context.chain, triggerNamespaceReady.eventID ]
            const targetRequests = await this.#getTargetRequests(context.namespace)
    ...
    async #triggerAll(targetRequests, context) {
        ...
        if (this.spec.targetRequestsCollector) {
            const collector = await this.#getTargetRequestsCollector()
            await collector.collect(
                {
                    ...context,
                    targetRequests:
                        results.map((ok, i) => ({ targetRequest: targetRequests[i], ok }))
                })
        }
    }
