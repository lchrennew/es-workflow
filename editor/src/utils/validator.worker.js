const getEventDisplayName = (eventName, sourceState, emitterOptions) => {
...
self.onmessage = (e) => {
  const { id, workflowConfig, prefetchers, emitterOptions } = e.data;
  const errors = [];
  const config = workflowConfig;

  // 1. kind 校验
  if (config.kind !== 'workflow') {
    errors.push('配置类型（kind）必须为工作流（"workflow"）。');
  }
  ...
  // 3. metadata 校验
  if (!config.metadata) {
    errors.push('缺少元数据配置（metadata）。');
  } else {
    if (!config.metadata.title) {
      errors.push('工作流显示标题（metadata.title）不能为空。');
    }
    ...
    if (config.metadata.tags && Array.isArray(config.metadata.tags)) {
      ...
      config.metadata.tags.forEach(tag => {
        if (!tagRegex.test(tag)) {
          errors.push(`标签格式不正确：“${tag}”，格式应当为“<类型>/<值>”。`);
        }
      });
    }
    ...
    if (config.metadata.version) {
      ...
      if (!objectIdRegex.test(config.metadata.version)) {
        errors.push(`版本号（metadata.version）格式不正确，必须是有效的二十四位标识符（ObjectID）。`);
      }
    }
  }
  ...
  if (!config.spec) {
    ...
    self.postMessage({ id, result: { valid: errors.length === 0, errors } });
    return;
  }
  ...
  states.forEach(state => {
    ...
    if (isInitial) {
      ...
      transitions.forEach(t => {
        if (t.event !== 'start') {
          errors.push(`开始节点的向外连线事件必须是“启动”，当前为“${getEventDisplayName(t.event, state, emitterOptions)}”。`);
        }
      });
    }
    ...
    transitions.forEach(t => {
      const eventDisplayName = getEventDisplayName(t.event, state, emitterOptions);
  ...
  states.forEach(state => {
    (state.transitions || []).forEach(t => {
      (t.targets || []).forEach(target => {
        if (forwardGraph[state.name] !== undefined) {
          forwardGraph[state.name].push(target.state);
        }
      });
    });
  });

  const reachableFromInitial = new Set(['initial']);
  const forwardQueue = ['initial'];

  while (forwardQueue.length > 0) {
    const current = forwardQueue.shift();
    const children = forwardGraph[current] || [];
    children.forEach(child => {
      if (!reachableFromInitial.has(child)) {
        reachableFromInitial.add(child);
  ...
  states.forEach(state => {
    if (state.name !== 'initial' && !reachableFromInitial.has(state.name)) {
      errors.push(`“${state.title || state.name}”节点无法从开始节点到达，属于孤立节点。`);
    }
  });

  // 可达性约束 B：除 end 外的所有状态，都必须存在一条“到达 end 的路径”
  const reverseGraph = {};
  states.forEach(s => {
    reverseGraph[s.name] = [];
  });
